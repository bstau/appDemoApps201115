Ext.define('Bloodbank.bloodbank.web.com.controller.healthcare.RegisterADonorMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.RegisterADonorMainController',
     init: function() {
     ...
     genderLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#gender');
                    scope.sender.controller.assignAllComboData(scope.sender, 'gender', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#gender');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#RegisterADonorTree');
               ...
               var compRef = this.view.down('#RegisterADonorTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.donorId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#RegisterADonor');
          var grid = this.view.down('#RegisterADonorGrid');
          var tree = this.view.down('#RegisterADonorTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#RegisterADonorTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var RegisterADonor = this.view.down('#RegisterADonorForm');
               RegisterADonor.reset();
               var gridPanel = this.view.down('#RegisterADonorGrid');
          ...
          var formPanel = this.getView().up().down('#RegisterADonorForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#RegisterADonorTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#RegisterADonor').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RegisterADonorGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#RegisterADonorForm').getViewModel().setData(responseData.response.data);
     },
