Ext.define('Bloodbank.bloodbank.web.com.controller.healthcare.RegisterACampMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.RegisterACampMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#RegisterACampTree');
               ...
               var compRef = this.view.down('#RegisterACampTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.campId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#RegisterACamp');
          var grid = this.view.down('#RegisterACampGrid');
          var tree = this.view.down('#RegisterACampTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#RegisterACampTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var RegisterACamp = this.view.down('#RegisterACampForm');
               RegisterACamp.reset();
               var gridPanel = this.view.down('#RegisterACampGrid');
          ...
          var formPanel = this.getView().up().down('#RegisterACampForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#RegisterACampTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#RegisterACamp').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RegisterACampGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#RegisterACampForm').getViewModel().setData(responseData.response.data);
     },
