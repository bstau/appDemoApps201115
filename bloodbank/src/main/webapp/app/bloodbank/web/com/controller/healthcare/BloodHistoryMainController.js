Ext.define('Bloodbank.bloodbank.web.com.controller.healthcare.BloodHistoryMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.BloodHistoryMainController',
     init: function() {
     ...
     bloodInfoLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/BloodInfo/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#bloodInfo');
                    scope.sender.controller.assignAllComboData(scope.sender, 'bloodInfo', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#BloodHistoryTree');
               ...
               var compRef = this.view.down('#BloodHistoryTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.bloodHistoryId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#BloodHistory');
          var grid = this.view.down('#BloodHistoryGrid');
          var tree = this.view.down('#BloodHistoryTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#BloodHistoryTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var BloodHistory = this.view.down('#BloodHistoryForm');
               BloodHistory.reset();
               var gridPanel = this.view.down('#BloodHistoryGrid');
          ...
          var formPanel = this.getView().up().down('#BloodHistoryForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#BloodHistoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BloodHistory').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BloodHistoryGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#BloodHistoryForm').getViewModel().setData(responseData.response.data);
     },
