Ext.define('Bloodbank.bloodbank.web.com.controller.healthcare.BloodStockMainController', {
     extend: 'Bloodbank.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.BloodStockMainController',
     init: function() {
          ...
          this.donarIdLoad();
          this.bloodGroupIdLoad();
     },
     donarIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#donarId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'donarId', response.responseText);
     ...
     bloodGroupIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#bloodGroupId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'bloodGroupId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#bloodGroupId');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#BloodStockTree');
               ...
               var compRef = this.view.down('#BloodStockTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.bloodStockId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#BloodStock');
          var grid = this.view.down('#BloodStockGrid');
          var tree = this.view.down('#BloodStockTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#BloodStockTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var BloodStock = this.view.down('#BloodStockForm');
               BloodStock.reset();
               var gridPanel = this.view.down('#BloodStockGrid');
          ...
          var formPanel = this.getView().up().down('#BloodStockForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#BloodStockTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BloodStock').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BloodStockGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#BloodStockForm').getViewModel().setData(responseData.response.data);
     },
