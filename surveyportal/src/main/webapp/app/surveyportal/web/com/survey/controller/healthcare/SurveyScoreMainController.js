Ext.define('Surveyportal.surveyportal.web.com.survey.controller.healthcare.SurveyScoreMainController', {
     extend: 'Surveyportal.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SurveyScoreMainController',
     ...
     groupTypeLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#groupType');
                    scope.sender.controller.assignAllComboData(scope.sender, 'groupType', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SurveyScoreTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SurveyScore');
          var grid = this.view.down('#SurveyScoreGrid');
          var tree = this.view.down('#SurveyScoreTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#SurveyScoreGrid');
               ...
               var formPanel = this.getView().up().down("#SurveyScore");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#SurveyScoreTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SurveyScore').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SurveyScoreGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.surveyScoreId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#SurveyScore').getViewModel().setData(responseData.response.data);
     },
