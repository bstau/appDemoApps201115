Ext.define('Surveyportal.surveyportal.web.com.survey.controller.healthcare.SurveyQuestionMainController', {
     extend: 'Surveyportal.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SurveyQuestionMainController',
     ...
     init: function() {
          ...
          this.surveyQuestionGroupIdLoad();
     },
     surveyQuestionGroupIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#surveyQuestionGroupId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'surveyQuestionGroupId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SurveyQuestionTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SurveyQuestion');
          var grid = this.view.down('#SurveyQuestionGrid');
          var tree = this.view.down('#SurveyQuestionTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#SurveyQuestionGrid');
               ...
               var formPanel = this.getView().up().down("#SurveyQuestion");
               ...
               var SurveyAnswerGrid = formPanel.down('#SurveyAnswerGrid');
               SurveyAnswerGrid.store.loadData(data.surveyAnswer);
               SurveyAnswerGrid.setSelection(0);
          }
     },
     /********************************Grid Controller Functions***********************************/
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#SurveyQuestionTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SurveyQuestion').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SurveyQuestionGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.surveyQuestionId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#SurveyQuestion').getViewModel().setData(responseData.response.data);
     },
     addSurveyAnswer: function(but) {
          var formSurveyAnswer = but.up();
          var gridSurveyAnswer = but.up().up().down('#SurveyAnswerGrid');
          if (formSurveyAnswer.isValid()) {
               var values = formSurveyAnswer.getValues();
               var modifiedValues = this.modifyFormValues(values, formSurveyAnswer);
               gridSurveyAnswer.getStore().add(modifiedValues);
               formSurveyAnswer.reset();
          }
     },
     onSurveyAnswerGridItemClick: function(cellModel, record, rowIndex, columnIndex, eOpts, comp) {
          var form;
          if (comp != null) {
               form = comp.up();
          } else {
               form = this.view.down('#SurveyAnswerForm');
          }
     ...
     resetForm: function(but) {
          ...
          var gridSurveyAnswer = this.view.down('#SurveyAnswerGrid');
          gridSurveyAnswer.getStore().removeAll();
     },
     saveForm: function(but, evt) {
          debugger;
          var addSurveyAnswerForm = but.up('form');
          var addSurveyAnswer = this.view.down('#SurveyAnswerGrid');
          if (!(addSurveyAnswerForm.isValid() || addSurveyAnswer.store.getCount() != 0)) {
               return;
          }
          this.addSurveyAnswer(addSurveyAnswerForm.down('button'));
          var addSurveyAnswer = this.view.down('#SurveyAnswerGrid');
          var store = addSurveyAnswer.store;
          this.onSurveyAnswerGridItemClick(null, store.data.items[0]);
          var form = but.up('form');
          if (!form.isValid()) {
               return;
          }
          var addSurveyAnswerForm = this.view.down('#SurveyAnswerForm');
          addSurveyAnswerForm.reset();
          var data = but.up('form').getViewModel().getData();
          var addSurveyAnswerBindLavel = addSurveyAnswer.bindLevel;
          delete data[addSurveyAnswerBindLavel];
          this.createObject(data, addSurveyAnswerBindLavel, this.fetchDataFromStore(addSurveyAnswer.getStore()));
