Ext.define('Surveyportal.surveyportal.web.com.survey.controller.healthcare.SurveyResultMainController', {
     extend: 'Surveyportal.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.SurveyResultMainController',
     init: function() {
          ...
          this.surveyQuestionIdLoad();
          this.surveyAnswerIdLoad();
     },
     surveyQuestionIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/SurveyQuestion/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#surveyQuestionId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'surveyQuestionId', response.responseText);
     ...
     surveyAnswerIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#surveyAnswerId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'surveyAnswerId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.surveyResultId;
          me = this;
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          debugger;
          var formPanel = this.getView().down('#SurveyResult');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SurveyResult').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SurveyResultGrid').store;
          gridStore.removeAll();
